...
import core.data.util.cv2r as cv2r
...
class Camera:
    def __init__(
        ...
        if extrinsic_matrix is not None:
            ...
            self.R = np.asarray(rot_world_to_cam, np.float32)  # w2c
            self.t = np.asarray(optical_center, np.float32)  # c2w
        ...
        self.world_up = np.asarray(world_up, np.float32)
    ...
    @staticmethod
    def from_numpy(R, t, world_up, intrinsic_matrix, distortion_coeffs):
        """ windness, 05162021
            create camera from numpy array
        """
        cam = Camera(world_up=world_up, intrinsic_matrix=intrinsic_matrix, distortion_coeffs=distortion_coeffs)
        cam.R = np.asarray(R, np.float32)
        cam.t = np.asarray(t, np.float32)
        return cam
    ...
    def rotate(self, yaw=0, pitch=0, roll=0):
        # windness, 05182021, force to use float32
        mat = transforms3d.euler.euler2mat(yaw, pitch, roll, 'ryxz').T.astype(np.float32)
        self.R = mat @ self.R
    ...
    def camera_to_world(self, points):
        """
        标准的 world-to-cam 转换公式：
            P_c = R_w2c @ P_w + t_w2c
        因此 cam-to-world：
            P_w = R_w2c^-1 @ (P_c - t) = R_w2c^-1 @ P_c + t_c2w
        因此：
            self.R: w2c的旋转矩阵
            self.t: c2w的平移向量，且转换公式为： P_w = R_c2w @ P_c + t_c2w
        """
        points = np.asarray(points, np.float32)
    ...
    def square_pixels(self):
        ...
        fmean = 0.5 * (fx + fy)
        # windness, 05182021, force to use float32
        multiplier = np.array([[fmean / fx, 0, 0], [0, fmean / fy, 0], [0, 0, 1]], dtype=np.float32)
    ...
    def turn_towards(self, target_image_point=None, target_world_point=None):
        ...
        new_z = unit_vec(target_world_point - self.t)  # new_z即 相机光心_w->target_world，即将相机的正面对准了target
